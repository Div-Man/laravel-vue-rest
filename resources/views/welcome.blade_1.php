...
/*
 axios.get('/sanctum/csrf-cookie').then(response => {
    axios.post('/api/v1/login', {
        ...
        const token = response2.data.access_token;
...
axios.defaults.headers.post['Authorization'] = `Bearer ${localStorage.getItem('Bearer')}`;
...
/*
 ...
 config => {
 config.headers['Authorization'] = `Bearer 16|6M9b64oG9tbfrayaco6Jt8bTL3dlZX47pI9lEIrw`;
 return config;
 },
 error => {
...
/*
 ...
 headers: {
 Authorization : `Bearer 16|6M9b64oG9tbfrayaco6Jt8bTL3dlZX47pI9lEIrw`
 }
 }
 )
 */
...
/*
 ...
 const token = response2.data.token; // Получение токена из ответа
 axios.defaults.headers.post['Authorization'] = `Bearer ${token}`; // Установка токена в заголовок
 // Выполнение дальнейших запросов с авторизацией
 });
 * 
 */
